...
const { Mentor, Student } = require("../schema/schema");

async function connectToDatabase() {
  try {
    await mongoose.connect(dbUrl, {
      useNewUrlParser: true, //This option tells Mongoose to use the new MongoDB connection string parser.
      useUnifiedTopology: true, //This option tells Mongoose to use the new Server Discovery and Monitoring engine.
    });
    console.log("Connected to MongoDB");
  } catch (error) {
    console.log("Error connecting to MongoDB:", error); //This logs an error message to the console if an error occurs during the database connection attempt
  }
}
...
router.put("/mentors/:mentorId/students/:studentId", async (req, res) => {
  try {
